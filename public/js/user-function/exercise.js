...
var itemSelected;
const userSendedListEl = document.querySelector('.user-sended-list');
const fileAnsListEl = document.querySelector('.ans-file-list');
// modal variable
...
function fileOptionAction() {
	...
	fileItemElement.forEach(function(item) {
		...
		optionBtn.onclick = function() {
            ...
            let itemSelectedDir = itemSelected.querySelector('.file-address').value;

            document.querySelector('.user-upload-selected').innerText = '';
            axios.get('get-list-file-uploaded', {
                params: {
                    fileName: itemSelectedDir
                }
            })
            .then(response => {
                listUserSend = response.data;
                console.log(response);
                if (listUserSend == '') {
                    openToast('.warningToast', 'Chưa có người dùng tải lên');
                }
                userSendedListEl.innerHTML = '';
                fileAnsListEl.innerHTML = '';
                Object.keys(listUserSend).forEach(user => {
                    
                    console.log(listUserSend[user]);


                    userSendedListEl.insertAdjacentHTML('beforeend', `
                    <div class="friend-accept-item card p-1 d-flex flex-row w-100 mt-2">
                        ...
                        <div class="friend-accept-item__more w-100 p-1 justify-content-center d-flex flex-column">
                            <div class="friend-accept-item__info d-flex flex-column">
                                <h5 class="friend-accept-item__info--name"><strong>${user}</strong></h5>
                            </div>
                        </div>
                    </div>
                    `);
                ...
                let listUserEl = userSendedListEl.querySelectorAll('.friend-accept-item');
                listUserEl.forEach(userEl => {
                    userEl.onclick = function() {
                        let userSendedName = userEl.querySelector('.friend-accept-item__info--name > strong').innerText;
                        
                        document.querySelector('.user-upload-selected').innerText = userSendedName;

                        showFileUploaded(listUserSend[userSendedName]);
                    }
                })
            })
            .catch(error => {
	...
	folderItemElement.forEach(function(item) {
		...
		optionBtn.onclick = function(e) {
            ...
            let itemSelectedDir = itemSelected.querySelector('.folder-address').value;
            console.log(itemSelectedDir);
		...
		arrFileUser.forEach(fileUser => {
			Object.entries(fileUser).forEach(([fileUploadedName, fileLink]) => {
				...
				if (fileTypeUploaded === 'docx' || fileTypeUploaded == 'doc') {
					fileAnsListEl.insertAdjacentHTML('beforeend', `
                        <div class="dirItem file-item d-flex card mt-1">
                            <input type="hidden" class="file-address" value="${fileLink}">
                            <div class="selector">
                                <div class="file-type">
                                    <img src="img/docs-icon/docx-icon.png" alt="">
                                </div>

                                <div class="file-info file-ans-to-down">
                                    <h6 class="file-info__name">${fileUploadedName}</h6>
                                    <div class="file-info__extends d-flex">
                                        <p class="file-info__type">${fileTypeUploaded} file</p>
                                    </div>
                                </div>
                            </div>

                            <a href="fileUploaded/${fileLink}" download="${fileUploadedName}" class="file-share">
                                <i class="option-btn download-btn bi bi-download"></i>					
                            </a>
                        </div>
					`);
					...
					fileAnsListEl.insertAdjacentHTML('beforeend', `
                        <div class="dirItem file-item d-flex card mt-1">
                            ...
                            <div class="selector">
                                <div class="file-type">
                                    <img src="img/docs-icon/pdf-icon.png" alt="">
                                </div>
                            ...
                            <a target="_blank" href="fileUploaded/${fileLink}" class="file-share">
                                <i class="option-btn download-btn bi bi-download"></i>					
                            </a>
                        </div>
					`);
					...
					fileAnsListEl.insertAdjacentHTML('beforeend', `
                        <div class="dirItem file-item d-flex card mt-1">
                            ...
                            <div class="selector">
                                <div class="file-type">
                                    <img src="img/docs-icon/img-icon.png" alt="">
                                </div>
